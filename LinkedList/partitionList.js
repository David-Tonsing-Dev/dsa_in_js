...
class LinkedList {
  ...
  partitionList(x) {
    ...
    let dummy1 = new Node();
    let dummy2 = new Node();
    let dummy1Temp = dummy1;
    let dummy2Temp = dummy2;

    while (temp) {
      prev = temp;
      temp = temp.next;
      if (prev.value < x) {
        dummy1Temp.next = prev;
        dummy1Temp = prev;
        dummy1Temp.next = null;
      } else {
        dummy2Temp.next = prev;
        dummy2Temp = prev;
        dummy2Temp.next = null;
      }
    }

    if (dummy1.next) {
      this.head = dummy1.next;
      dummy1Temp.next = dummy2.next;
    } else {
      this.head = dummy2.next;
    }

    if (dummy2.next) {
      this.tail = dummy2Temp;
    } else {
      this.tail = dummy1Temp;
    }
    dummy1.next = null;
    dummy2.next = null;
  }
